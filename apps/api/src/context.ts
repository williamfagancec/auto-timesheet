...
export async function createContext({ req, res }: CreateFastifyContextOptions) {
  ...
  if (authHeader && userIdHeader) {
    ...
    if (validateApiKey(authHeader)) {
      try {
        const apiUser = await getUserForApiKey(userIdHeader as string)

        // Convert to Lucia User type
        user = {
          id: apiUser.id,
          email: apiUser.email,
          name: apiUser.name,
          rmUserId: apiUser.rmUserId,
        }
        authMethod = 'api_key'

        console.log('[Context] API key authentication successful:', {
          userId: user.id,
          email: user.email,
        })
      } catch (error) {
        console.error('[Context] API key user validation failed:', error)
        // Don't set user - will fall through to session check
      }
    } else {
      console.warn('[Context] Invalid API key provided')
    }
  }

  // Fall back to session-based authentication if API key auth didn't succeed
  if (!user) {
