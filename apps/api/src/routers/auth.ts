...
export const authRouter = router({
  ...
  login: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        // Better-Auth returns user and automatically sets session cookies via headers
  ...
  googleOAuth: publicProcedure.mutation(async () => {
    try {
      ...
      const redirectUrl = `${process.env.API_URL || 'http://localhost:3001'}/api/auth/signin/google`
