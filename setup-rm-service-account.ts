...
try {
  ...
  if (!rajToken) {
    ...
    console.log('\nUsage: npx tsx setup-rm-service-account.ts <RM_API_TOKEN> <RM_USER_ID>')
  ...
  console.log(`  Token User: ${userName} (${currentUser.email})`)
  ...
  if (!williamRmUserId) {
    ...
    console.log('\nThen run:')
    console.log(`npx tsx setup-rm-service-account.ts <RM_API_TOKEN> <RM_USER_ID>`)
    process.exit(1)
  }

  const williamUserId = Number.parseInt(williamRmUserId)
  if (Number.isNaN(williamUserId) || williamUserId <= 0) {
    console.error('❌ Invalid RM user ID. Must be a positive number.')
    process.exit(1)
  }
